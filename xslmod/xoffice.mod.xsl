...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xtlcon="http://www.xtpxlib.nl/ns/container"
  ...
  <xsl:function name="xtlxo:get-file-root-from-relationship-type" as="element()?">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="rels-dir" as="xs:string" select="xtlc:href-path(xtlc:href-path(xtlxo:get-rels-href($basefile-href)))"/>
        <xsl:variable name="target-ref" as="xs:string" select="xtlc:href-canonical(xtlxo:doc-href(($rels-dir, $relationship/@Target)))"/>
        <!-- Always retrieve the file root as mandatory, because once here, it must be there! -->
        <xsl:sequence select="xtlxo:get-file-root($extracted-office-xml, $target-ref, true())"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
  ...
  <xsl:function name="xtlxo:get-rels-href" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="basedir" as="xs:string" select="xtlc:href-path($basefile-href)"/>
        <xsl:variable name="rels-filename" as="xs:string" select="concat(xtlc:href-name($basefile-href), $rels-extension)"/>
        <xsl:sequence select="xtlxo:doc-href(($basedir, $rels-subdir, $rels-filename))"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
