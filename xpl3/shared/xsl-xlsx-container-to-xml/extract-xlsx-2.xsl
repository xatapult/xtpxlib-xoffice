...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xtlxo="http://www.xtpxlib.nl/ns/xoffice" xmlns="http://www.xtpxlib.nl/ns/xoffice"
  ...
  <xsl:template match="xtlxo:row">
    ...
    <xsl:variable name="cells" as="element(xtlxo:cell)*">
      <xsl:for-each select="xtlxo:cell">
        <xsl:if test="some $child in xtlxo:* satisfies (string($child) ne '')">
          <xsl:copy copy-namespaces="no">
            <xsl:copy-of select="@*" copy-namespaces="no"/>
            <xsl:for-each select="xtlxo:*">
