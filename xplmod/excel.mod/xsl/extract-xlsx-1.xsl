...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:template match="/*">
    <workbook href="{/*/@href-source-zip}" timestamp="{current-dateTime()}">
      ...
      <xsl:variable name="workbook" as="element(mso-wb:workbook)"
        select="xtlxo:get-file-root-from-relationship-type($extracted-office-xml, '', $xtlxo:relationship-type-main-document, true())"/>
      <xsl:variable name="workbook-href" as="xs:string" select="xtlxo:get-href($workbook)"/>
      
      <!-- Get the (optional) shared string table: -->
      <xsl:variable name="shared-strings" as="element(mso-wb:sst)?"
        select="xtlxo:get-file-root-from-relationship-type($extracted-office-xml, $workbook-href, $xtlxo:relationship-type-shared-strings, false())"/>
      ...
      <xsl:for-each select="$workbook/mso-wb:sheets/mso-wb:sheet">
        ...
        <xsl:call-template name="process-worksheet">
          <xsl:with-param name="worksheet"
            select="xtlxo:get-file-root-from-relationship-id($extracted-office-xml, $workbook-href, @mso-rels:id, true())"/>
  ...
  <xsl:template name="process-worksheet">
    ...
    <worksheet name="{$name}">
      ...
      <xsl:variable name="worksheet-href" as="xs:string" select="xtlxo:get-href($worksheet)"/>
      <xsl:variable name="comments-root" as="element(mso-wb:comments)?"
        select="xtlxo:get-file-root-from-relationship-type($extracted-office-xml, $worksheet-href, $xtlxo:relationship-type-comments, false())"/>
